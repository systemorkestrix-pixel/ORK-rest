...
def handle(event: DomainEvent) -> None:
    ...
    try:
        ...
        active_assignment = db.execute(
            ...
            .where(
                DeliveryAssignment.order_id == int(order_id),
                DeliveryAssignment.status == "assigned",
            )
            .order_by(desc(DeliveryAssignment.id))
        ).scalars().first()
        if active_assignment is not None:
            push_driver_assignment_update(
                db=db,
                driver_id=int(active_assignment.driver_id),
        ...
        logger.exception("Failed to handle ORDER_READY for order_id=%s", order_id)
    finally:
        db.close()
